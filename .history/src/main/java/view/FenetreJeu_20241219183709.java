...
public class FenetreJeu extends JFrame {
    ...
    private void ajouterAnimationPion(int colonne) {
        ...
        int ligne = -1; // Initialisation de la variable ligne
        for (int i = 5; i >= 0; i--) {
            if (jeu.getGrille().getCase(i, colonne).isEmpty()) {
                ligne = i; // Mise à jour de ligne si une case vide est trouvée
                break; // Sortir de la boucle une fois la case trouvée
            }
        }
    
        // Si une case vide a été trouvée, commencer l'animation
        if (ligne != -1) {
            // Couleur du pion actuel
            String couleurPion = jeu.getJoueurActuel().getCouleur();
            Color couleur = couleurPion.equals("Rouge") ? Color.RED : Color.YELLOW;
            ...
            new Thread(() -> {
                for (int i = 0; i <= ligne; i++) {
                    try {
                        ...
                        casesGrille[i][colonne].setBackground(couleur);
                        // Pause pour créer l'effet d'animation
                        Thread.sleep(200); // Délai de 200 ms entre chaque étape
                        // Réinitialiser la case à vide si on n'est pas à la dernière ligne
                        if (i < ligne) {
                            casesGrille[i][colonne].setBackground(Color.WHITE);
                        }
